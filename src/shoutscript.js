...
import * as g_ from './Gradule-web';
import * as ssvm_ from './ssvm';

import fsWeb from '../typed/fs-web';

const
  _qp = (color) => function(string, styles, bold = false, italic = false) {
  ...
  _getFNArgs = (fn = () => { }) => {
    ...
    let argStr = fnStr.includes('=>')
      ? fnStr.slice(fnStr.indexOf('(') + 1, fnStr.lastIndexOf(')'))
      : fnStr.match(/function\s*\(([^)]*)\)/)[1];
  ...
  _getFNBody = (fn = () => { }) => {
    let fnStr = fn.toString();
    let fnBody = fnStr.includes('=>')
      ? fnStr.slice(fnStr.indexOf('{') + 1, fnStr.lastIndexOf('}'))
      : fnStr.match(/function\s*\(([^)]*)\)\s*\{([^]*)\}/)[2];
    return fnBody;
  },
  ...
  _static = {
    ...
    main_color: _qp(
      [...g_.preset.amethyst, ...g_.preset.cherryblossoms]
        .sort(() => Math.random() - 0.5)
    ),

    error_color: _qp(
      [...g_.preset.kyoto, ...g_.preset.wiretap]
        .sort(() => Math.random() - 0.5)
    ),
  ...
  compiled = Object.entries(shouts).map(([key, val]) => {
    if (!key.startsWith('_')) {
